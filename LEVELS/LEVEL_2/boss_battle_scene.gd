...
func addEnemies():
	...
	for parent in $enemies.get_children():
		var enemy = load("res://LEVELS/LEVEL_2/boss.tscn").instantiate()
...
func enemyTurn(enemy: Node2D):
	...
	if isDefending:
		damage = 0
	var damageText = "%s pulses and deals %d damage" % [enemy.name, damage]
...
func attack(enemy: int) -> void:
	...
	var damage = (Global.playerStats["attack"] - sceneCharacters[foe]["defense"])
	var damageText = "null deals %d damage to %s" % [damage, foe.name]
	setLabelText(damageText)
	await get_tree().create_timer(2.).timeout
	sceneCharacters[foe]["current health"] -= damage
